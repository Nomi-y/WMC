...
import * as plf from "../plf.ts";
import { alben as alben_pojo } from "../alben.ts"; // werden in test 3 mit durch array mit Song-Objekten ersetzt
    ...
    for (const alb_no in Object.keys(expected)) {
        ...
        for (const i in expected[alb_no]) {
            assertEquals(
                ...
                album`Song ${i} von Album ${alb_no} sollte ${songs[song_index].title
                    } sein`,
            );
        }
    }
